...
- Container snapshots are exported as checksum-verified OCI archives. Once the host copy is durable, the duplicate image/archive is released from the appliance and imported only when a restore needs it. VM snapshots use QCOW2 internal snapshots. Restores also restore saved policies and connection definitions.
...
For an agent-only update that preserves the existing immutable base disk, run the following from WSL (Go 1.18 or newer):
