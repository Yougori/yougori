...
def collect_build_material():
    ...
    with tempfile.TemporaryDirectory(prefix="source-material-", dir=WORK) as temporary:
        ...
        for relative in sorted(set(candidates)):
            if not (relative in frontend_files or relative in ("LICENSE", "COPYING", "NOTICE", "COMMERCIAL_LICENSE.md", "docs/licensing.md", "docs/rebuilding-third-party.md",
...
def report():
    ...
    selected = [name for name in tracked if name in ("LICENSE", "COPYING", "NOTICE", "COMMERCIAL_LICENSE.md", "docs/licensing.md", "package.json", "package-lock.json",
