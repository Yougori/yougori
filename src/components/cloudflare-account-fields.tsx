...
import type { CloudflareDraft } from "@/lib/cloudflare-account"
...
export function CloudflareAccountFields({ environmentId, port, value, onChange, busy, perform, refreshKey }: {
  environmentId: string; port: number; value: CloudflareDraft; onChange: Dispatch<SetStateAction<CloudflareDraft>>; busy: boolean; perform(action: () => Promise<unknown>): Promise<void>; refreshKey: string
}) {
  ...
  useEffect(() => {
    ...
    void workspaceApi.savedCloudflare(environmentId, port).then(result => {
      ...
      onChange(current => ({ ...current, hostname: current.hostname || result.hostname, localPort: current.localPort || (result.hostPort ? String(result.hostPort) : "") }))
    }).catch(() => { if (!disposed) setLoadError("Could not read saved credentials. You can paste a token for this session or retry.") })
    return () => { disposed = true }
  }, [environmentId, port, value.mode, revision, onChange, refreshKey])
  ...
  return <section aria-label="Cloudflare account options" className="flex flex-col gap-3 rounded-lg border p-3">
    ...
    {value.mode === "account" ? <>
      ...
      <Label className="flex items-center gap-2 text-xs"><Checkbox disabled={busy} checked={value.remember} onCheckedChange={checked => onChange(current => ({ ...current, remember: checked === true }))} />Remember in this PC’s credential vault</Label>
